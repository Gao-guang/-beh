...
from dbehavior.btree.branch import GuardSequence
...
class GCReEntry(ConditionLeaf):
    ...
    def condition(self):
        ...
        # self.get_bb().re_entry and \


class GCInitialEntry(ConditionLeaf):

    def condition(self):
        # return not self.get_bb().re_entry and \
...
class GCState2Freeze(ConditionLeaf):
    ...
    def condition(self):
        if self.get_bb().enemy_free_kick:
            return False
...
class GCState2Execute(ConditionLeaf):

    def condition(self):
        return self.get_bb().gc_info.state2Execute
...
class GCKickOffSupporterDelay(ConditionLeaf):
    ...
    def condition(self):
        ...
        return gc_info.kickoff and \
               'Supporter' in self.get_bb().param.role and \
               gc_info.state is GCInfo.STATE_PLAYING and \
               gc_info.secondaryTime != 0  # and \
...
class GCNonKickOffPlaying(ConditionLeaf):
    ...
    def condition(self):
        gc_info = self.get_bb().gc_info
        return not gc_info.kickoff and \
               'Striker' in self.get_bb().param.role and \
               gc_info.state is GCInfo.STATE_PLAYING and \
               gc_info.secondaryTime > 3  # and \
        #    self.get_bb().timer_gc_playing <= 10
